...
For Cloudflare Workers Builds, `wrangler.jsonc` includes a custom build step so **`npx wrangler versions upload` builds OpenNext automatically** before upload.

Default dashboard settings work:

- **Build command:** *(leave empty — wrangler runs `npm run build:cloudflare` via `wrangler.jsonc` `build.command`)*
- **Deploy command:** `npx wrangler versions upload`

Optional alternatives:

- **Deploy command:** `npm run upload:cloudflare` (explicit build + upload in one npm script)
- **Split:** build command `npm run build:cloudflare`, deploy `npx wrangler versions upload`

`.open-next/assets` is generated by OpenNext and is not committed to git.
