...
For Cloudflare Workers Builds, set **one** of these:

**Recommended (single deploy step — builds then uploads):**

- **Build command:** *(leave empty)*
- **Deploy command:** `npm run upload:cloudflare`
...
Do **not** run `wrangler versions upload` without `build:cloudflare` first — `.open-next/assets` is generated by OpenNext and is not committed to git.
