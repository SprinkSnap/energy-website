/**
 * Headless responsiveness check for Natural Air Infiltration Other Factors section.
 */
...
async function run() {
  ...
  await page.waitForSelector("#infiltration-other-factors-mount .infiltration-other-factors-section", { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(() => {
      const labelsRequired = ["Terrain", "Anemometer Height", "Leakage fractions", "Ceilings", "Walls", "Floors"];
      const viewportWidth = window.innerWidth;
      const section = document.querySelector("#infiltration-other-factors-mount .infiltration-other-factors-section");
      ...
      const clippedLabels = [...(section?.querySelectorAll(".infiltration-tab-stack .field > span") || [])]
        .filter(isVisible)
      ...
      const controlSelector = ".infiltration-tab-stack input:not([type='checkbox']):not([type='hidden']), .infiltration-tab-stack select";
      ...
      const fields = [...(section?.querySelectorAll(".infiltration-tab-stack .field") || [])].filter(isVisible);
      const oneColumn =
        viewportWidth >= 640
          ? true
          : fields.length < 2
