/**
 * Verify Ontario Reference House Program has one outer card and no inner group borders.
 */
...
async function run() {
  ...
  await page.waitForSelector('.program-ontario-ref-layout[data-program-mode="ontarioRef"]', { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const outerBorderMatch = bl && outer
        ...
        : false;
      const innerGroups = [...(layout?.querySelectorAll(".program-ontario-ref-layout > .spec-group") || [])];
      ...
      const nestedSectionHeadings = [...(layout?.querySelectorAll(
        ".program-site-admin-group > h4, .program-vermiculite-group > h4, .program-resiliency-group > h4, .program-rur-comments-group > h4",
      ...
      const checkboxUsable = (() => {
        ...
        return inputs.length >= 13 && inputs.every((el) => el.getBoundingClientRect().height >= 16);
      })();
      const selectBorder = (() => {
      ...
      const inputBorder = (() => {
        const input = layout?.querySelector('.program-options-grid input[type="text"]');
        if (!input) return false;
        const s = getComputedStyle(input);
        return s.borderTopWidth !== "0px" && s.borderTopStyle !== "none";
      })();
      const toolbar = document.getElementById("programMode")?.value || "";
      const hasResiliency = (layout?.textContent || "").includes("Smart thermostats");
      const hasRurComments = (layout?.textContent || "").includes("RUR comments");
      return {
        overflow,
        outerBorderMatch,
        innerGroupBorders,
    ...
    const pass =
      ...
      metrics.nestedSectionHeadings.length === 0 &&
      metrics.checkboxUsable &&
      metrics.selectBorder &&
      metrics.inputBorder &&
      metrics.toolbar === "ontarioRef" &&
      metrics.hasResiliency &&
      metrics.hasRurComments;
