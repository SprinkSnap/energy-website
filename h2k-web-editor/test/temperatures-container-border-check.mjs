/**
 * Verify Temperatures has one outer section-card and no nested group borders.
 */
...
async function run() {
  ...
  await page.waitForSelector("#screen-systems-temperatures .temperatures-main-group", { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const section = document.querySelector("#screen-systems-temperatures .temperatures-section.section-card");
      const baseLoads = document.querySelector("#screen-systems-base-loads .base-loads-section.section-card");
      const layout = section?.querySelector(".temperatures-spec-layout");
      const mainGroup = layout?.querySelector(".temperatures-main-group");
      const cardStyle = (el) => {
        if (!el) return null;
      ...
      const mainFloorsHeading = mainGroup?.querySelector("h4")?.textContent?.trim() || "";
      ...
      const inputBorder = (() => {
        const input = mainGroup?.querySelector('input[data-xml-path*="@daytimeHeatingSetPoint"]');
      ...
      const nestedSectionHeadings = innerGroups
    ...
    const pass =
      ...
      metrics.mainFloorsHeading === "Main floors" &&
      metrics.missingLabels.length === 0 &&
      metrics.inputBorder &&
      metrics.selectBorder &&
      metrics.nestedSectionHeadings.includes("Basement") &&
      metrics.nestedSectionHeadings.includes("Sizing Indoor Design Temperatures");
