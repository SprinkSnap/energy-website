/**
 * Headless responsiveness check for Window Tightness section.
 */
...
const REQUIRED_LABELS = ["Window tightness", "Leakage value (L/s·m²)"];
...
async function run() {
  ...
  await page.waitForSelector("#screen-house-tightness .tightness-section", { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate((labelsRequired) => {
      const section = document.querySelector("#screen-house-tightness .tightness-section");
      ...
      const fields = [...(section?.querySelectorAll(".field") || [])].filter(isVisible);
      const oneColumn =
        fields.length < 2
          ? true
          : fields.every((el, i) => {
