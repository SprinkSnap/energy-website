/**
 * Headless responsiveness check for Units & Mode section.
 */
...
const REQUIRED_LABELS = ["Program", "Mixed Use", "Imperial", "Metric", "English", "French"];
...
async function run() {
  ...
  await page.waitForSelector("#screen-house-unit-mode .unit-mode-section", { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(
      (viewportWidth, labelsRequired) => {
        const section = document.querySelector("#screen-house-unit-mode .unit-mode-section");
        ...
        const unitRadios = [...(section?.querySelectorAll('input[name="unitModeSection"]') || [])].filter(isVisible);
        const languageRadios = [...(section?.querySelectorAll('input[name="unitModeLanguage"]') || [])].filter(isVisible);
        const radioOneColumn = (radios) =>
          viewportWidth < 640
            ? radios.every((el, i) => {
                if (i === 0) return true;
                const prev = radios[i - 1].getBoundingClientRect();
        ...
        return {
          ...
          missingLabels,
          tappableControls,
          unitsOneColumn: radioOneColumn(unitRadios),
          languageOneColumn: radioOneColumn(languageRadios),
          unitRadios: unitRadios.length,
          languageRadios: languageRadios.length,
          hasProgramSelect: Boolean(section?.querySelector("[data-unit-mode-program]")),
    ...
    const pass =
      ...
      metrics.unitsOneColumn &&
      metrics.languageOneColumn &&
      metrics.unitRadios === 2 &&
      metrics.languageRadios === 2 &&
      metrics.hasProgramSelect;
