/**
 * Headless responsiveness check for House Units & Mode section.
 */
...
const REQUIRED_LABELS = ["Metric", "Imperial", "US", "Programs", "Display Units"];
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(
      (viewportWidth, labelsRequired) => {
        ...
        const unitRadios = [...(section?.querySelectorAll('input[name="unitModeDisplayUnits"]') || [])].filter(isVisible);
        const radioOneColumn =
          viewportWidth < 640
            ? unitRadios.every((el, i) => {
                if (i === 0) return true;
                const prev = unitRadios[i - 1].getBoundingClientRect();
                const cur = el.getBoundingClientRect();
                return cur.top >= prev.bottom - 2;
              })
            : true;
        const programsSelect = section?.querySelector("[data-unit-mode-programs]");
        const programsValue = programsSelect?.options?.[programsSelect.selectedIndex]?.textContent?.trim() || "";
        return {
          overflow,
          clippedLabels,
          clippedInputs,
          ...
          hasProgramsSelect: Boolean(programsSelect),
          programsValue,
    ...
    const pass =
      ...
      metrics.radioOneColumn &&
      metrics.unitRadios === 3 &&
      metrics.hasProgramsSelect &&
      metrics.programsValue.length > 0;
