/**
 * Verify Natural Air Infiltration has one outer section-card and no nested card borders.
 */
...
async function run() {
  ...
  await page.waitForSelector("#infiltration-specifications-mount .infiltration-tab-stack .spec-group", { timeout: 90000 });
  ...
  for (const width of WIDTHS) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const section = document.querySelector("#screen-systems-natural-air-infiltration .infiltration-section.section-card");
      ...
      const innerGroups = [...(section?.querySelectorAll(".infiltration-tab-stack .spec-group") || [])];
      const innerGroupBorders = innerGroups.filter((el) => {
        const s = getComputedStyle(el);
        return s.borderTopWidth !== "0px" && s.borderTopStyle !== "none";
      }).length;
      const tabButtons = [...section?.querySelectorAll(".infiltration-local-nav .base-loads-local-nav-item") || []];
      const tabsEqualWidth =
        tabButtons.length >= 2 &&
        Math.abs(tabButtons[0].getBoundingClientRect().width - tabButtons[1].getBoundingClientRect().width) < 3;
      const houseHeading = !!section?.querySelector('.infiltration-tab-stack .spec-group > h4');
      const inputBorder = (() => {
        const input = section?.querySelector(".infiltration-tab-stack input:not([type='checkbox'])");
